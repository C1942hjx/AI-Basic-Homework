...
def VectorSearch(query,top_k=8,id=0):
    ...
    vector_storage = QdrantStorage(
        ...
        path="storage_amazon_review",
        collection_name="Amazon 图书评论数据"+str(id)
    )
...
def AmazonReviewSearch(query,top_k=8):

    #提取关键信息

    print("正在准备...")

    msg1="你是一个书籍推荐助手，给出用户的问题，请提取出关键信息并整理，注意书籍名称、书籍特征等信息，接下来需要在 Amazon **评论**向量数据集中进行检索，请用**书籍或作者所在国家的语言**和英语分别输出，同时**不要输出除关键词以外多余的内容干扰向量信息检索**"
    ...
    msg2="你是一个书籍推荐助手，给出询问以及在 Amazon **评论**向量数据集中的检索结果，需要整理检索结果，提取答案，注意过滤掉无用或者错误的信息，避免重复的信息，回答用户"
